...
impl Model {
    pub fn next_wave(&mut self) {
        ...
        self.area_effects.push(AreaEffect {
            ...
            radius: thread_rng().gen_range(5.0..15.0),
    ...
    fn generate_wave(&self) -> Wave {
        ...
        let groups_count = rng.gen_range(max_groups.max(2) - 1..max_groups.max(1) + 1);
        let mut wave = Wave {
            groups: Vec::with_capacity(groups_count),
        };
        for _ in 0..groups_count {
            let max_enemies = (self.current_stage as f32).sqrt().floor() as usize;
            let enemies_count = rng.gen_range(max_enemies.max(3) - 2..max_enemies.max(1) + 1);
            ...
            for _ in 0..enemies_count {
                let mut random = rng.gen_range(0.0..1.0);
                let mut enemy = None;
